...
GRID_LEVELS = 7  # 그리드 레벨 수
GRID_RANGE_PERCENT = 3.0  # 그리드 범위 ±3%
PROFIT_PER_GRID = 0.008  # 그리드당 목표 수익 0.8%
...
for idx, candle in enumerate(ohlcv):
    ...
    if low <= stop_loss_price and not stopped:
        ...
        if coin > 0:
            ...
            print(f"[{timestamp}] 🛑 스톱로스 발동: {coin:.4f}개 @ {sell_price:,.2f}원 → {krw_received:,.0f}원 (수수료 {fee:.0f}원)")
        ...
        if low <= level_price and position['buy_price'] is None:
            if krw >= MIN_ORDER_KRW:
                ...
                if krw_spent <= krw:
                    ...
                    if len(trades) <= 20 or len(trades) % 10 == 0:
                        print(f"[{timestamp}] 🟢 매수 L{level_idx+1}: {buy_amount:.4f}개 @ {buy_price:,.2f}원 (수수료 {fee:.0f}원)")
        ...
        # 매도 체크 (포지션 있고 목표가 도달)
            ...
            if high >= target_sell_price:
                ...
                if sell_price * sell_amount >= MIN_ORDER_KRW:
                    ...
                    if len(trades) <= 20 or len(trades) % 10 == 0:
                        print(f"[{timestamp}] 🔴 매도 L{level_idx+1}: {sell_amount:.4f}개 @ {sell_price:,.2f}원 → 수익 {profit:,.0f}원 ({profit_pct:+.2f}%)")
...
if coin > 0:
    ...
    krw += krw_final
    print(f"\n[최종 정산] 잔여 코인 {coin:.4f}개 @ {sell_price:,.2f}원 → {krw_final:,.0f}원")
    coin = 0
...
if len(trades) > 0:
    ...
    for trade in trades[-20:]:
        ...
        if trade['type'] == 'SELL' and 'profit' in trade:
            print(f"{time_str} {type_emoji} {trade['type']:4s} L{trade['level']} | {trade['amount']:.4f}개 @ {trade['price']:,.2f}원 | 수익: {trade['profit']:+,.0f}원 ({trade['profit_pct']:+.2f}%)")
        else:
            level = f"L{trade['level']}" if 'level' in trade else "ALL"
            print(f"{time_str} {type_emoji} {trade['type']:4s} {level:3s} | {trade['amount']:.4f}개 @ {trade['price']:,.2f}원")
